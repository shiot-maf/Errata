...
| `/saved` | 저장함 — 담아둔 교정·표현. 복습 일정이 함께 표시된다 |
...
담아둔 것은 담아두기만 하면 그냥 목록입니다. 저장함의 세 종류를 이렇게 나눕니다.

| 종류 | 덱에 | 이유 |
| --- | --- | --- |
| `phrase` "더 자연스럽게" 제안 | **넣는다** | 틀린 적이 없어서 `mistakes`에 없다 — 여태 한 번도 복습되지 않았다 |
| `correction` 교정 북마크 | 안 넣는다 | 같은 실수가 이미 덱에 있다. 대신 그 실수에 별표를 달아 순서를 앞당긴다 |
| `selection` 드래그 발췌 | 못 넣는다 | `back`이 비어 있어 물어볼 정답이 없다 |

원본 일기를 지워 `mistake`가 사라진 `correction`은 저장함 쪽이 유일한 사본이므로
그때는 덱에 들어갑니다.

표현은 틀린 게 아니라서 묻는 말도 다릅니다 — "둘 중 맞는 쪽" 대신 "둘 중 더
자연스러운 쪽", "정답" 대신 "담아둔 표현", 원래 표현에 취소선을 긋지 않습니다.
그리고 하루 몫에서 **세 자리를 표현에 떼어둡니다.** 표현은 심각도가 가장 낮아서
순서를 심각도로만 매기면 밀린 실수 몇 개만 있어도 영영 차례가 오지 않습니다.
...
`Mistake`와 `SavedItem` 문서에 `box`와 `dueAt`만 더했습니다. 없으면 상자 0 ·
지금 만기로 읽으므로 예전에 쌓인 것도 그대로 굴러 들어옵니다.

덱이 다루는 타입은 `ReviewItem`입니다(`src/lib/review/item.ts`). 예전에는 덱이
곧 `Mistake` 목록이라 "복습할 수 있는 건 실수뿐"이라는 가정이 타입에 박혀
있었습니다. 지금은 일정·채점·집계가 둘을 구별하지 않고, 어디서 왔는지는
`source`가 들고 있다가 저장할 때만 갈립니다.
...
- **지표는 판권줄에** — 레벨과 다음 레벨까지의 진행, 불꽃이 붙은 스트릭,
  ...
  아니라 끊길 위기 쪽입니다.
- **펼친 자리는 `/profile`** — 판권줄에 한 줄로 눌려 있던 것을 한 장으로
  폅니다. 탭 여섯 자리 중 하나를 설정 대신 여기에 줬습니다 — 설정은 한 번
  맞춰두면 다시 열 일이 드물지만 레벨·스트릭·도장은 매일 봅니다. 설정은
  이 화면 안으로 들어갔습니다. 맨 위에는 1년치 잔디가 섭니다. 레벨과 칭호는
  앱이 매긴 값이지만 그 격자는 실제로 쓴 날이 그대로 찍힌 것이라서요. 칭호는 반짝이는 배지가 아니라 빨간펜 도장으로 찍습니다. 화면
  컴포넌트(`ProfileSheet`)는 Firestore도 `useAuth`도 부르지 않고 받은 값만
  그리므로, 나중에 남이 보는 공개 프로필이 생겨도 그대로 재사용됩니다.
...
이름(`displayName`)은 겹쳐도 됩니다. 하지만 친구를 찾고 프로필을 가리키려면
겹치지 않는 열쇠가 하나 필요합니다. 그게 핸들(`@my_handle`)입니다.

Firestore에는 "이 필드는 유일해야 한다"는 제약이 없습니다. 그래서 **이름
자체를 문서 id로 쓰는 컬렉션**을 두고 먼저 만든 사람이 갖습니다.

```
handles/{handle}  →  { uid, createdAt }
diaryUsers/{uid}  →  { handle, ... }
```

확인과 생성을 **트랜잭션 하나로 묶기 때문에**, 둘이 같은 순간에 같은 이름을
눌러도 한 명만 갖습니다(`src/lib/firebase/handles.ts`). 규칙에서 `get`은
누구에게나 열어둡니다 — 가입 화면에서 로그인 전에 "쓸 수 있는 이름인가"를
물어야 하기 때문입니다. 대신 `list`는 막습니다. 열어두면 그게 곧 사용자
목록이 됩니다.

형식은 좁게 잡았습니다(`src/lib/handle.ts`). 영문 소문자·숫자·밑줄, 3~20자,
영문자로 시작. 대소문자를 섞어 받으면 `Errata`와 `errata`가 다른 사람이 되고
그건 사람을 속이기 좋은 구조입니다. `admin`, `settings`, `profile` 같은
주소로 쓸 말들은 미리 비워뒀습니다.

핸들은 가입할 때 선택으로 받고(그 사이 누가 채가면 계정만 만들어지고 프로필
화면이 다시 물어봅니다), 프로필의 이름 아래에서 언제든 정하거나 바꿀 수
있습니다. 바꾸는 것은 지우고 다시 잡는 것이라, 옛 이름은 다른 사람이
가져갈 수 있습니다.
...
설정에서 **주소·모델·키** 세 가지만 넣으면 됩니다. 모델 이름은 코드에 적는
순간부터 낡습니다 — 처음 붙일 때 적어둔 `gemini-2.5-flash`는 배포하자마자
404가 났습니다. 그래서 앱은 목록을 들고 있지 않고, 제공자의 `/models`에 직접
물어봅니다. 프리셋을 고르거나 키를 저장하면 그 목록을 바로 불러와서 모델 칸이
드롭다운으로 바뀝니다.
